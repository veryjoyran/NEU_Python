...
class DatabaseReader:
    ...
    def preprocess_price(self):
        ...
        self.data['price_numeric'] = self.data['price'].str.extract(r'(\d+\.?\d*)').astype(float)
...
class DatabaseViewer:
    ...
    def display_data(self, data):
        ...
        for _, row in data.iterrows():
            self.table.insert("", tk.END, values=(row['room_type'], row['area'], row['floor'],
                                                  row['orientation'], row['build_year'], row['owner_name'],
                                                  row['address'], row['description'], row['price']))
